...
export default async function TrekPrintable() {
  const supabase = createAdminSupabase();
  const { data: families } = await supabase
    .from("families")
    .select(
      "id, registrant_name, email, phone, country_code, trek_jul18, trek_jul18_pax"
    )
    .eq("trek_jul18", true)
    .order("registrant_name", { ascending: true });

  const list = families || [];
  const total = list.reduce((s, f) => s + (f.trek_jul18_pax || 0), 0);
      ...
      <div className="flex items-center justify-between print:hidden">
        <div>
          <h1 className="text-2xl font-semibold text-zinc-900">
            Trek roster — 18 July 2026, 7:30 AM
          </h1>
          <p className="text-sm text-zinc-500 mt-1">
            {list.length} families · {total} pax total
          </p>
        </div>
        <PrintButton />
      </div>
            ...
            <tbody>
              {list.map((f, i) => (
                <tr key={f.id} className="border-b border-zinc-100">
                  ...
                  <td className="py-2 font-semibold">{f.trek_jul18_pax}</td>
                </tr>
              ))}
